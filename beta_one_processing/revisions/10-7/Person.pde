...
PImage picture;
PVector pos,avg=new PVector(0,0),flowavg=new PVector(0,0);
color head;
...
float sumy=0;
float count=0;
 PVector flowVec=new PVector(0,0) , center=new PVector(video.width/2,video.height/2);
...
if (search){
  ...
  avg.set(0,0);
  flowavg.set(0,0);
for(int y=headRect.y;y<ey;y+=2){
    for(int x=headRect.x;x<ex;x+=2){
  ...
  //stroke(255);
  for(int y=headRect.y-40;y<ey+40;y+=20){
    for(int x=headRect.x-40;x<ex+40;x+=20){
      if(x<video.width &&y<video.height&&y>0&&x>0){
         flowVec = opencv.getFlowAt(x,y);
       flowVec.sub(aveFlow);
       flowVec.mult(10);
       float flowval=flowVec.magSq();
       if(flowval>ave){
         
       //line(x,y,x+flowVec.x,y+flowVec.y);
         flow+=flowval;
       flowavg.add(x+flowVec.x,y+flowVec.y);
       flowcount++;}
      }}}
  if(count<(0*colorthreshold/10) || count>66*coloredpixels){
    if(unsure==false){
  unsure=true;
  //println("help");
countdown=millis();}
    ...
    if(count>0){
      //println(count);
      if((flow/flowcount)>aveFlow.magSq()){
avg.div(count);
flowavg.div(flowcount);
pos.add(avg);
pos.div(2);
println(aveFlow.magSq());
if(aveFlow.magSq()<10){
  if(flowavg.x<video.width && flowavg.y<video.height){
    
    
//line(pos.x,pos.y,pos.x+flowavg.x/2,pos.y+flowavg.y/2);
pos.add(flowavg);
pos.div(2);
ellipse(flowavg.x,flowavg.y,20,20);
}}else{pos.add(center);
pos.div(2);}
