...
boolean scanface=true,esp=false,algerithm2=false;//wether it starts folloing a face
...
OpenCV opencv;
//object for opencv
int timedelay=0,selected=0,widthface=6,boundsx=225,boundsy=150;
...
if(Capture.list().length>1){
  ...
  if(cameras[3].equals("HK 2M CAM #2")){
  video=new Capture(this,width,height-300,"HK 2M CAM #2");
  }print("went throught");
  if(i==Capture.list().length){
  video=new Capture(this,width,height-300);}
  }
}else{
video=new Capture(this,width,height-300); }
